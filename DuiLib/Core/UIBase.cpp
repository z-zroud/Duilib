...
{
    //如果存在父窗口类名，但无法注册父窗口则创建窗口失败
    if( GetSuperClassName() != NULL && !RegisterSuperclass() ) return NULL;
    //如果不存在父窗口，但无法注册该窗口，则创建窗口失败
    if( GetSuperClassName() == NULL && !RegisterWindowClass() ) return NULL;
    m_hWnd = ::CreateWindowEx(dwExStyle,    //窗口的扩展风格
        GetWindowClassName(),   //注册的窗口类名
        pstrName,   //窗口名称
        dwStyle,    //窗口风格
        x,  //窗口水平位置X
        y,  //窗口垂直位置
        ...
        hwndParent,     //父窗口句柄
        hMenu,  //菜单句柄
        CPaintManagerUI::GetInstance(),     //应用程序实例句柄
        this);  //指向窗口的创建数据
    ASSERT(m_hWnd!=NULL);
    return m_hWnd;
}
...
bool CWindowWnd::RegisterWindowClass()
{
    ...
    wc.cbWndExtra = 0;
    wc.hIcon = NULL;
    wc.lpfnWndProc = CWindowWnd::__WndProc; //默认创建窗口的
...
/********************************************************************
* 该默认的窗口消息处理函数仅仅处理窗口关闭消息，并将关闭消息传递给
* 父窗口，其他消息均由HandleMessage函数处理
*********************************************************************/
...
/*******************************************************************
* SendMessage直接被窗口消息过程处理函数捕获，直到处理完成才返回
********************************************************************/
...
/*******************************************************************
* 将消息发送到应用消息队列中，立刻返回，之后由消息队列分发该消息给
* 窗口消息处理函数处理
********************************************************************/
